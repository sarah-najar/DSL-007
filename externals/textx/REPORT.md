...
En pratique, TextX a été choisi car il reste léger (pas d’outillage lourd), aligné sur le méta-modèle ArduinoML (bricks digitaux/analogiques, états, transitions conditionnelles et temporisées, actions buzzer/LCD) et permet de régénérer immédiatement du Wiring validé sur carte. Scénarios éprouvés en vrai hardware : `lcd_status` (LCD BUS1, bouton D9, LED D10), `multi_state_alarm` (cycle buzzer/LED), `temporal_after_push` (tempo de 800 ms), `toggle` et les alarmes simple/double bouton. Exemple minimal lisible : `application demo sensor b:9 actuator led:10 state off { set led to LOW when b is HIGH goto on } state on { set led to HIGH when b is HIGH goto off }`, puis `python textx/main.py demo.aml` → sketch prêt à flasher. Limites actuelles : pas de commentaires dans les `.aml`, mapping LCD figé sur BUS1 (pas de BUS2/I²C), pins restreintes (8–12 sans LCD, 9–12 avec LCD), LCD sur une seule ligne, validation limitée à unicité/pins/longueur message/état initial.
